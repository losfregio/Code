...
cur.executescript('''
DROP TABLE IF EXISTS Utility_Prices_Aitor_NoGasCCL;

        
 CREATE TABLE Utility_Prices_Aitor_NoGasCCL (
     id     INTEGER NOT NULL,
     DNO INTEGER,
     Voltage INTEGER,
     Ele  FLOAT,
    ...
    for Voltage in range(3):
        ...
        for count in range(len(time_id)):
            ...
            if not math.isnan(final_ele):
                cur.execute('''INSERT INTO Utility_Prices_Aitor_NoGasCCL(id, DNO, Voltage, Ele, Gas, Ele_exp) VALUES({time_id}, {dno},{Voltage},{Value},{Value_Gas},{Value_exp})'''.format(time_id = time_count, dno = DNO, Voltage = Voltage, Value = final_ele, Value_Gas = final_gas, Value_exp = final_ele_exp))   
